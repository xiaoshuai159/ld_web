  ...
  <div class="app-container demo-tabs">
      ...
      <el-col :span="22">
        <div class="tableClass">
          <el-table v-loading="loading"
            ...
            <el-table-column prop="chara_name" label="特征名称" sortable min-width="150" align="center"
              show-overflow-tooltip />
            <el-table-column prop="chara_type" label="特征类型" min-width="100" align="center" show-overflow-tooltip />
            <el-table-column prop="create_time" label="创建时间" sortable min-width="170" align="center"
              show-overflow-tooltip />
            <el-table-column prop="star" label="星级评价" sortable min-width="180" align="center">
              <template #default="{ row }">
                <el-rate v-model="row.star" show-score text-color="#ff9900" score-template="{value}星"
                  @change="starChange(row)" />
              </template>
            </el-table-column>
            <el-table-column prop="chara_level" label="相关资产重要等级" sortable min-width="180" align="center"
              show-overflow-tooltip />
    ...
    <el-dialog v-model="xqDialogVisible" title="详情信息" width="35%">
      <div class="xqDialog">
        ...
        <span>特征编号</span><span>{{ curXqData.chara_id }}</span><br />
        <span>特征名称</span><span>{{ curXqData.chara_name }}</span><br />
        <span>特征类型</span><span>{{ curXqData.chara_type }}</span><br />
        <span>星级评价</span><span> <el-rate v-model="curXqData.star" disabled show-score text-color="#ff9900"
            score-template="{value}星" /> </span><br />
        <span>特征资产重要等级</span><span>{{ curXqData.chara_level }}</span><br />

        <span>查询语法</span><span>{{ curXqData.query_info }}</span><br />
    ...
    <el-dialog v-model="xjDialogVisible" title="新建" width="30%">
      <el-form ref="xjForm" :model="curXjData" :rules="rules" label-width="140px">
        <el-form-item label="特征名称" prop="chara_name">
          <el-input v-model="curXjData.chara_name" placeholder="请输入特征名称" style="width: 220px"
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item label="特征类型" prop="chara_type">
          <el-select v-model="curXjData.chara_type" placeholder="请选择" style="width: 220px"
            @keyup.enter="handleSubmit(xjForm)">
            <el-option v-for="item in tzlxOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="相关资产重要等级" prop="chara_level">
          <el-select v-model="curXjData.chara_level" placeholder="请选择" style="width: 220px"
            @keyup.enter="handleSubmit(xjForm)">
            <el-option v-for="item in zycdOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="厂商名称" prop="ma_name">
          <el-input v-model="curXjData.ma_name" placeholder="请输入厂商名称" style="width: 220px"
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item label="产品名称" prop="product_name">
          <el-input v-model="curXjData.product_name" placeholder="请输入产品名称" style="width: 220px"
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item label="产品版本" prop="product_version">
          <el-input v-model="curXjData.product_version" placeholder="请输入产品版本" style="width: 220px"
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item label="查询语法" prop="query_info">
          <el-input v-model="curXjData.query_info" placeholder="请输入查询语法" style="width: 220px"
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item label="特征描述" prop="description">
          <el-input v-model="curXjData.description" placeholder="请输入描述信息" style="width: 220px" type="textarea"
            :rows="3" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="bjDialogVisible" title="编辑" width="30%">
      <el-form ref="bjForm" :model="curBjData" :rules="rules" label-width="140px">
        <el-form-item label="特征id" prop="chara_name">
          <el-input v-model="curBjData.chara_id" disabled placeholder="请输入特征id" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="特征名称" prop="chara_name">
          <el-input v-model="curBjData.chara_name" placeholder="请输入特征名称" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="特征类型" prop="chara_type">
          <el-select v-model="curBjData.chara_type" placeholder="请选择" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)">
            <el-option v-for="item in tzlxOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="相关资产重要等级" prop="chara_level">
          <el-select v-model="curBjData.chara_level" placeholder="请选择" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)">
            <el-option v-for="item in zycdOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="厂商名称" prop="ma_name">
          <el-input v-model="curBjData.ma_name" placeholder="请输入厂商名称" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="产品名称" prop="product_name">
          <el-input v-model="curBjData.product_name" placeholder="请输入产品名称" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="产品版本" prop="product_version">
          <el-input v-model="curBjData.product_version" placeholder="请输入产品版本" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="查询语法" prop="query_info">
          <el-input v-model="curBjData.query_info" placeholder="请输入查询语法" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="特征描述" prop="description">
          <el-input v-model="curBjData.description" placeholder="请输入特征名称" style="width: 220px" type="textarea"
            :rows="3" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    if (valid) {
      ...
      if (res.code == 200) {
        if (res.data.exists == 0) {
          ...
          curXjData.chara_type = ''
          curXjData.star = ''
          curXjData.chara_level = ''
          curXjData.query_info = ''
