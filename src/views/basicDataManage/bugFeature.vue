<template>
  <div class="app-container demo-tabs">
      ...
      <el-row style="margin-top: 10px">
        ...
        <el-col :span="22">
          <div>
            ...
            <span style="margin-left: 10px; display: inline-block; width: 70px; text-align: right">星级评价：</span>
      ...
      <el-col :span="22">
        <div class="tableClass">
          <el-table v-loading="loading"
            ...
            <el-table-column prop="star" label="星级评价" min-width="180" align="center" show-overflow-tooltip>
    ...
    <el-dialog v-model="xqDialogVisible" title="详情信息" width="35%">
      <div class="xqDialog">
        <span>漏洞编号</span><span>{{ curXqData.c_id }}</span><br />
        <span>特征编号</span><span>{{ curXqData.chara_id }}</span><br />
        <span>漏洞名称</span><span>{{ curXqData.chara_name }}</span><br />
        ...
        <span>危害程度</span><span>{{ curXqData.vul_level }}</span><br />
        <span>漏洞规则定义</span><span>{{ curXqData.vul_rule }}</span><br />
    ...
    <el-dialog v-model="xjDialogVisible" title="新建" width="30%">
      <el-form ref="xjForm" :model="curXjData" :rules="rules" label-width="140px">
        <el-form-item label="CNVD/CVE编号" prop="c_id">
          <el-input v-model="curXjData.c_id" placeholder="请输入CNVD/CVE编号" style="width: 220px"
        ...
        <el-form-item label="漏洞规则定义" prop="vul_rule">
          <el-input v-model="curXjData.vul_rule" placeholder="请输入漏洞规则定义" style="width: 220px"
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
    ...
    <el-dialog v-model="bjDialogVisible" title="编辑" width="30%">
      <el-form ref="bjForm" :model="curBjData" :rules="rules" label-width="140px">
        ...
        <el-form-item label="CNVD/CVE编号" prop="c_id">
          <el-input v-model="curBjData.c_id" disabled placeholder="请输入CNVD/CVE编号" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="特征名称" prop="chara_name">
        ...
        <el-form-item label="漏洞规则定义" prop="vul_rule">
          <el-input v-model="curBjData.vul_rule" placeholder="请输入漏洞规则定义" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="返回值" prop="vul_payload">
          <el-input v-model="curBjData.vul_payload" placeholder="请输入描述信息" style="width: 220px" type="textarea"
            :rows="3" />
        </el-form-item>
      </el-form>
      <template #footer>
