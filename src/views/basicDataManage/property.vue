  ...
  <div class="app-container demo-tabs">
      ...
      <el-row style="margin-top: 10px">
        ...
        <el-col :span="22">
          <div style="display: inline-block">
            ...
            <span style="margin-left: 10px">星级评价：</span>
      ...
      <el-col :span="22">
        <div class="tableClass">
          <el-table v-loading="loading"
            ...
            <el-table-column prop="asset_name" label="资产名称" min-width="150" align="center" show-overflow-tooltip
              sortable />
            <el-table-column prop="asset_type" label="资产类型" min-width="100" align="center" show-overflow-tooltip />
            <el-table-column prop="area" label="资产归属地区" min-width="120" align="center" show-overflow-tooltip />
            <el-table-column prop="create_time" label="资产录入时间" min-width="180" align="center" show-overflow-tooltip
              sortable />
            <!-- <el-table-column prop="asset_unit" label="资产归属单位" min-width="180" align="center" show-overflow-tooltip
              sortable /> -->
            <el-table-column prop="star" label="星级评价" min-width="130" align="center" show-overflow-tooltip sortable>
              <template #default="{ row }">
                <el-rate v-model="row.star" show-score text-color="#ff9900" score-template="{value}星"
    ...
    <el-dialog v-model="xqDialogVisible" title="详情信息" width="35%">
      <div class="xqDialog">
        ...
        <span>特征id</span><span>{{ curXqData.chara_id }}</span><br />
        <span>相关资产特征名称</span><span>{{ curXqData.chara_name }}</span><br />
        <span>资产编号</span><span>{{ curXqData.asset_id }}</span><br />
        <span>资产名称</span><span>{{ curXqData.asset_name }}</span><br />
        <span>资产类型</span><span>{{ curXqData.asset_type }}</span><br />
        <!-- <span>资产归属单位</span><span>{{ curXqData.asset_unit }}</span><br /> -->
        <span>星级评价</span><span>{{ curXqData.star }}</span><br />
        <span>资产来源</span><span>{{ curXqData.asset_from }}</span><br />

        <span>备案信息</span><span>{{ curXqData.icp }}</span><br />
        <span>资产归属地区</span><span>{{ curXqData.area }}</span><br />
        <span>资产内容</span><span>{{ curXqData.asset_li }}</span><br />
        <span>资产录入时间</span><span>{{ curXqData.create_time }}</span><br />
        <span>标签</span><span>{{ curXqData.labels }}</span><br />
      </div>
    </el-dialog>
    <el-dialog v-model="xjDialogVisible" title="新建" width="30%">
      <el-form ref="xjForm" :model="curXjData" :rules="rules" label-width="140px">
        <el-form-item label="资产名称" prop="asset_name">
          <el-input v-model="curXjData.asset_name" placeholder="请输入特征名称" style="width: 220px"
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item label="资产类型" prop="asset_type">
          <el-select v-model="curXjData.asset_type" placeholder="请选择资产类型" style="width: 220px"
            @keyup.enter="handleSubmit(xjForm)">
        ...
        <el-form-item label="星级评价" prop="star">
          <el-select v-model="curXjData.star" placeholder="请选择" style="width: 220px"
            @keyup.enter="handleSubmit(xjForm)">
        ...
        <el-form-item label="资产重要程度" prop="asset_level">
          <el-select v-model="curXjData.asset_level" placeholder="请选择" style="width: 220px"
            @keyup.enter="handleSubmit(xjForm)">
            <el-option v-for="item in zycdOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="资产归属地区" prop="area">
          <el-input v-model="curXjData.area" placeholder="请输入描述信息" style="width: 220px" />
        </el-form-item>
        <el-form-item label="资产信息" prop="asset_li">
          <el-input v-model="curXjData.asset_li" placeholder="请输入资产信息" style="width: 220px"
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        ...
        <el-form-item label="厂商名称" prop="ma_name">
          <el-input v-model="curXjData.ma_name" placeholder="请输入厂商名称" style="width: 220px" disabled
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item label="产品名称" prop="product_name">
          <el-input v-model="curXjData.product_name" placeholder="请输入产品名称" style="width: 220px" disabled
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item label="产品版本" prop="product_version">
          <el-input v-model="curXjData.product_version" placeholder="请输入产品版本" style="width: 220px" disabled
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="bjDialogVisible" title="编辑" width="30%">
      <el-form ref="bjForm" :model="curBjData" :rules="rules" label-width="140px">
        <el-form-item label="资产id" prop="asset_id">
          <el-input v-model="curBjData.asset_id" disabled placeholder="请输入资产id" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="资产名称" prop="asset_name">
          <el-input v-model="curBjData.asset_name" placeholder="请输入特征名称" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="资产类型" prop="asset_type">
          <el-select v-model="curBjData.asset_type" placeholder="请选择" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)">
            <el-option v-for="item in tzlxOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="星级评价" prop="star">
          <el-select v-model="curBjData.star" placeholder="请选择" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)">
            <el-option v-for="item in tzdjOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="相关资产重要程度" prop="asset_level">
          <el-select v-model="curBjData.asset_level" placeholder="请选择" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)">
            <el-option v-for="item in zycdOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="资产归属地区" prop="area">
          <el-input v-model="curBjData.area" placeholder="请输入资产归属地区" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <!-- <el-form-item label="资产归属单位" prop="asset_unit">
        ...
        <el-form-item label="资产备案信息" prop="icp">
          <el-input v-model="curBjData.icp" placeholder="请输入备案信息" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="资产详细信息" prop="asset_li">
          <el-input v-model="curBjData.asset_li" placeholder="请输入资产详细信息" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="标签" prop="labels">
          <el-input v-model="curBjData.labels" placeholder="请输入标签" style="width: 220px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item  label="相关资产特征名称" prop="chara_name">
          ...
          <el-input v-model="curBjData.chara_id" placeholder="请输入特征id" style="width: 220px" disabled
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="厂商名称" prop="ma_name">
          <el-input v-model="curBjData.ma_name" placeholder="请输入厂商名称" style="width: 220px" disabled
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="产品名称" prop="product_name">
          <el-input v-model="curBjData.product_name" placeholder="请输入产品名称" style="width: 220px" disabled
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="产品版本" prop="product_version">
          <el-input v-model="curBjData.product_version" placeholder="请输入产品版本" style="width: 220px" disabled
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { ref, reactive, onBeforeMount, Ref, onMounted, inject, watch } from 'vue'
...
const rules = ref({
  ...
  star: [{ required: true, message: '请选择星级评价', trigger: 'change' }],
