    ...
    <div class="app-container demo-tabs">
        ...
        <el-row>
          ...
          <el-col :span="22">
            <div style="display: inline-block">
              <!-- <span>任务编号：</span>
              <el-input v-model="ldmcInput" style="width: 300px" placeholder="请输入" /> -->
              <span>任务名称：</span>
              <el-input v-model="task_name" style="width: 220px" placeholder="请输入" />
  
              <!-- <span style="margin-left: 10px">任务类型：</span>
              <el-select v-model="task_type" placeholder="请选择" style="width: 300px">
      ...
      <el-dialog v-model="xqDialogVisible" title="详情信息" width="35%">
        <div v-if="curXqData.task_type === 'detect'" class="xqDialog">
          ...
          <span>探测目标</span><span>{{ curXqData.taskContent.ip_targets.length>0? curXqData.taskContent.ip_targets.join(", "):"无"}}</span><br />
      ...
      <el-dialog v-model="xjDialogVisible" title="新建" width="35%">
        <el-form ref="xjForm" :model="curXjData" :rules="rules" label-width="180px">
          ...
          <!-- <el-form-item label="探测目标类型" prop="ipSourceType" v-if="curXjData.task_type === 'detect'">
            <el-select v-model="curXjData.ipSourceType" placeholder="请选择" style="width: 300px"
              @keyup.enter="handleSubmit(xjForm)">
              <el-option v-for="item in ipSourceTypeOptions" :key="item.value" :label="item.label"
                :value="item.value"></el-option>
            </el-select>
          </el-form-item> -->
          <!-- <el-form-item v-if="curXjData.task_type === 'detect' && curXjData.ipSourceType == 1" label="探测目标"
            prop="ip_targets">
          ...
          <!-- <el-form-item label="ip分组" prop="targetGroups"
            ...
            <el-input v-model="curXjData.targetGroups" placeholder="多个ip，以英文逗号分割" style="width: 300px"
              @keyup.enter="handleSubmit(xjForm)" />
          </el-form-item> -->
          <!-- <el-form-item label="端口目标类型" prop="pocType" v-if="curXjData.task_type === 'detect'">
            <el-select v-model="curXjData.pocType" placeholder="请选择" style="width: 300px"
              @keyup.enter="handleSubmit(xjForm)">
              <el-option v-for="item in ipSourceTypeOptions" :key="item.value" :label="item.label"
                :value="item.value"></el-option>
            </el-select>
          ...
          <!-- <el-form-item label="poc分组" prop="pocGroups" v-if="curXjData.task_type === 'detect' && curXjData.pocType == 2">
            <el-input v-model="curXjData.pocGroups" placeholder="多个poc，以英文逗号分割" style="width: 300px"
              @keyup.enter="handleSubmit(xjForm)" />
          </el-form-item> -->
      ...
      <el-dialog v-model="tcjgDialogVisible" title="结果" width="70%">
        <div>
          <el-table
            ...
            <el-table-column type="index" width="55" />
            <el-table-column prop="unit" label="资产归属单位" min-width="130" align="center" show-overflow-tooltip />
            <el-table-column prop="industry" label="资产归属行业" min-width="130" align="center" show-overflow-tooltip />
  ...
  const handleSubmit = (formEl: FormInstance | undefined) => {
    ...
    formEl.validate(async (valid) => {
      if (valid) {
        ...
        if (curXjData.task_type == 'detect') {
            console.log(curAllpoc);
            let formData1 = {
            ...
            formData.append('json_data',JSON.stringify(formData1))
            if (fileList && fileList.value[0]) {
              formData.append('file', fileList.value[0].raw)
            }
            // formData.append('file',JSON.stringify(formData1))
          }
