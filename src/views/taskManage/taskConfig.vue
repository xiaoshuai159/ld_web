<template>
  <div class="app-container demo-tabs">
    <div class="my-header">漏洞资产发现</div>
          ...
          <el-table v-loading="loading"
            ...
            <el-table-column prop="operator" label="操作" min-width="320px" align="center" fixed="right">
              <template #default="scope">
                ...
                <el-button type="success" size="small" link @click="qdClick(scope.row)"
                  v-if="scope.row.state != 1 && scope.row.state != 2 && scope.row.state != 3" >
                  {{ scope.row.state <= 3 ? '启动' : '重新启动' }}</el-button>
                <el-button type="danger" size="small" link @click="tzClick(scope.row)"
                  v-if="scope.row.state == 2" > 停止 </el-button>
                <el-button type="primary" size="small" link @click="editHandler(scope.row)"
                  v-if="scope.row.state != 1 && scope.row.state != 2 && scope.row.state != 3" >
                  编辑
                </el-button>
                <el-button type="danger" size="small" link @click="del(scope.row)"> 删除 </el-button>
                <el-button v-if="scope.row.state == 5" type="success" size="small" link @click="jgClick(scope.row)">
                  结果查看 </el-button>
                <el-button v-if="scope.row.state == 5" type="primary" size="small" link @click="jgxzClick(scope.row)">
                  {{ scope.row.downloadButtonText || '结果下载' }} </el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
    ...
    <el-dialog v-model="xjDialogVisible" title="新建" width="40%">
      <el-form ref="xjForm" :model="curXjData" :rules="rules" label-width="180px">
        <el-form-item label="任务名称" prop="task_name">
          <el-input v-model="curXjData.task_name" placeholder="请输入任务名称" style="width: 300px"
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item label="任务类型" prop="task_type">
          <el-select v-model="curXjData.task_type" placeholder="请选择任务类型" style="width: 300px"
            @keyup.enter="handleSubmit(xjForm)">
            <el-option v-for="item in rwlxOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item v-if="curXjData.task_type === 'search'" label="资产特征" prop="chara_name">
          <el-select v-model="curXjData.chara_name" filterable remote reserve-keyword placeholder="请输入并选择资产特征"
            remote-show-suffix :remote-method="remoteMethod" :loading="selectLoading" style="width: 300px"
            @keyup.enter="handleSubmit(xjForm)">
        ...
        <el-form-item v-if="curXjData.task_type === 'search'" label="相关厂商" prop="ma_name">
          <el-input v-model="curXjData.ma_name" placeholder="请输入相关厂商名称" style="width: 300px" disabled
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item v-if="curXjData.task_type === 'search'" label="相关产品名称" prop="product_name">
          <el-input v-model="curXjData.product_name" placeholder="请输入相关产品名称" style="width: 300px" disabled
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item v-if="curXjData.task_type === 'search'" label="相关产品版本" prop="product_version">
          <el-input v-model="curXjData.product_version" placeholder="请输入相关产品版本" style="width: 300px" disabled
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item v-if="curXjData.task_type === 'search'" label="资产特征编号" prop="chara_id">
          <el-input v-model="curXjData.chara_id" placeholder="请输入资产特征编号" style="width: 300px" disabled
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item v-if="curXjData.task_type === 'search'" label="特征对应语法" prop="query_syntax">
          <el-input v-model="curXjData.query_syntax" placeholder="请输入特征对应语法" style="width: 300px" disabled
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item v-if="curXjData.task_type === 'search'" label="资产特征等级" prop="chara_level">
          <el-input v-model="curXjData.chara_level" placeholder="请输入资产特征等级" style="width: 300px" disabled
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <!-- detect -->
        <el-form-item v-if="curXjData.task_type === 'detect'" label="探测网络模式" prop="internetMode">
          <el-select v-model="curXjData.internetMode" placeholder="请选择探测网络模式" style="width: 300px"
            @keyup.enter="handleSubmit(xjForm)">
        ...
        <el-form-item v-if="curXjData.task_type === 'detect'" label="任务优先级" prop="priority">
          <el-select v-model="curXjData.priority" placeholder="请选择任务优先级" style="width: 300px"
            @keyup.enter="handleSubmit(xjForm)">
        ...
        <el-form-item v-if="curXjData.task_type === 'detect'" label="网络类型" prop="source">
          <el-select v-model="curXjData.source" placeholder="请选择网络类型" style="width: 300px"
            @keyup.enter="handleSubmit(xjForm)">
        ...
        <el-form-item v-if="curXjData.task_type === 'detect'" label="失败后是否重新调度" prop="reissue">
          <el-select v-model="curXjData.reissue" placeholder="请选择" style="width: 300px"
            @keyup.enter="handleSubmit(xjForm)">
        ...
        <el-form-item v-if="curXjData.task_type === 'detect' && curXjData.reissue == 9" label="重新调度次数"
          prop="dispatchTimes">
          <el-input v-model="curXjData.dispatchTimes" placeholder="请输入重新调度次数" style="width: 300px"
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item v-if="curXjData.task_type === 'detect'" label="任务标签" prop="labelIds">
          <el-input v-model="curXjData.labelIds" placeholder="多个任务标签，以英文逗号分割" style="width: 300px"
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        ...
        <el-form-item v-if="curXjData.task_type === 'detect' && curXjData.ipSourceType == 1" label="探测端口目标"
          prop="port_targets">
          <el-input v-model="curXjData.port_targets" placeholder="多个port，以英文逗号分割" style="width: 300px"
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        ...
        <el-form-item v-if="curXjData.task_type === 'detect'" label="探测脚本" prop="pocs">
          <el-select v-model="curXjData.pocs" filterable remote reserve-keyword placeholder="请输入并探测脚本"
            remote-show-suffix :remote-method="remoteMethod3" :loading="selectLoading" style="width: 300px"
            @keyup.enter="handleSubmit(xjForm)">
        ...
        <el-form-item v-if="curXjData.task_type === 'detect'" label="漏扫白名单" prop="blacklist">
          <el-input v-model="curXjData.blacklist" placeholder="多个ip，以英文逗号分割" style="width: 300px"
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item v-if="curXjData.task_type === 'detect'" label="探测节点" prop="nodeType">
          <el-select v-model="curXjData.nodeType" placeholder="请选择探测节点" style="width: 300px"
            @keyup.enter="handleSubmit(xjForm)">
        ...
        <el-form-item v-if="curXjData.task_type === 'detect'" label="使用节点数量" prop="nodeCount">
          <el-input v-model="curXjData.nodeCount" placeholder="请输入使用节点数量" style="width: 300px"
            @keyup.enter="handleSubmit(xjForm)" />
        </el-form-item>
        <el-form-item v-if="curXjData.task_type === 'detect'" label="探测目标">
            <!-- <el-input v-model="curXjData.ip_targets" placeholder="多个ip，以英文逗号分割" style="width: 300px"
              @keyup.enter="handleSubmit(xjForm)" /> -->
            <el-upload v-model:file-list="fileList" style="width: 300px" class="upload-demo" drag action="#"
              :auto-upload="false" :limit="1">
              <el-icon class="el-icon--upload"><upload-filled /></el-icon>
              <div class="el-upload__text"> 将txt文件拖拽到这里<em>上传</em> </div>
            </el-upload>
          </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="bjDialogVisible" title="编辑" width="38%">
      <el-form ref="bjForm" :model="curBjData" :rules="rules" label-width="180px">
        <el-form-item label="任务名称" prop="task_name">
          <el-input v-model="curBjData.task_name" placeholder="请输入任务名称" style="width: 300px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item label="任务类型" prop="task_type">
          <el-select v-model="curBjData.task_type" placeholder="请选择任务类型" style="width: 300px" disabled
            @keyup.enter="handleSubmit2(bjForm)">
            <el-option v-for="item in rwlxOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item v-if="curBjData.task_type === 'search'" label="资产特征" prop="chara_name">
          <el-select v-model="curBjData.chara_name" filterable remote reserve-keyword placeholder="请输入并选择资产特征"
            remote-show-suffix :remote-method="remoteMethod2" :loading="selectLoading" style="width: 300px"
            @keyup.enter="handleSubmit2(bjForm)">
            <el-option v-for="item in curXjData.zctzOption" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="curBjData.task_type === 'search'" label="相关厂商" prop="ma_name">
          <el-input v-model="curBjData.ma_name" placeholder="请输入相关厂商名称" style="width: 300px" disabled
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item v-if="curBjData.task_type === 'search'" label="相关产品名称" prop="product_name">
          <el-input v-model="curBjData.product_name" placeholder="请输入相关产品名称" style="width: 300px" disabled
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item v-if="curBjData.task_type === 'search'" label="相关产品版本" prop="product_version">
          <el-input v-model="curBjData.product_version" placeholder="请输入相关产品版本" style="width: 300px" disabled
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item v-if="curBjData.task_type === 'search'" label="资产特征编号" prop="chara_id">
          <el-input v-model="curBjData.chara_id" placeholder="请输入资产特征编号" style="width: 300px" disabled
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item v-if="curBjData.task_type === 'search'" label="特征对应语法" prop="query_syntax">
          <el-input v-model="curBjData.query_syntax" placeholder="请输入特征对应语法" style="width: 300px" disabled
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item v-if="curBjData.task_type === 'search'" label="资产特征等级" prop="chara_level">
          <el-input v-model="curBjData.chara_level" placeholder="请输入资产特征等级" style="width: 300px" disabled
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>

        <!-- detect -->
        <el-form-item v-if="curBjData.task_type === 'detect'" label="探测网络模式" prop="internetMode">
          <el-select v-model="curBjData.internetMode" placeholder="请选择探测网络模式" style="width: 300px"
            @keyup.enter="handleSubmit2(bjForm)">
            <el-option v-for="item in internetModeOptions" :key="item.value" :label="item.label"
              :value="item.value"></el-option>
          </el-select>
        ...
        <el-form-item v-if="curBjData.task_type === 'detect'" label="任务优先级" prop="priority">
          <el-select v-model="curBjData.priority" placeholder="请选择任务优先级" style="width: 300px"
            @keyup.enter="handleSubmit2(bjForm)">
            <el-option v-for="item in priorityOptions" :key="item.value" :label="item.label"
              :value="item.value"></el-option>
          </el-select>
        ...
        <el-form-item v-if="curBjData.task_type === 'detect'" label="网络类型" prop="source">
          <el-select v-model="curBjData.source" placeholder="请选择网络类型" style="width: 300px"
            @keyup.enter="handleSubmit2(bjForm)">
            <el-option v-for="item in sourceOptions" :key="item.value" :label="item.label"
              :value="item.value"></el-option>
          </el-select>
        ...
        <el-form-item v-if="curBjData.task_type === 'detect'" label="失败后是否重新调度" prop="reissue">
          <el-select v-model="curBjData.reissue" placeholder="请选择" style="width: 300px"
            @keyup.enter="handleSubmit2(bjForm)">
            <el-option v-for="item in reissueOptions" :key="item.value" :label="item.label"
              :value="item.value"></el-option>
          </el-select>
        ...
        <el-form-item v-if="curBjData.task_type === 'detect' && curBjData.reissue == 9" label="重新调度次数"
          prop="dispatchTimes">
          <el-input v-model="curBjData.dispatchTimes" placeholder="请输入重新调度次数" style="width: 300px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item v-if="curBjData.task_type === 'detect'" label="任务标签" prop="labelIds">
          <el-input v-model="curBjData.labelIds" placeholder="多个任务标签，以英文逗号分割" style="width: 300px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item v-if="curBjData.task_type === 'detect' && curBjData.ipSourceType == 1" label="探测端口目标"
          prop="port_targets">
          <el-input v-model="curBjData.port_targets" placeholder="多个port，以英文逗号分割" style="width: 300px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        ...
        <el-form-item v-if="curBjData.task_type === 'detect'" label="漏扫白名单" prop="blacklist">
          <el-input v-model="curBjData.blacklist" placeholder="多个ip，以英文逗号分割" style="width: 300px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item v-if="curBjData.task_type === 'detect'" label="探测节点" prop="nodeType">
          <el-select v-model="curBjData.nodeType" placeholder="请选择探测节点" style="width: 300px"
            @keyup.enter="handleSubmit2(bjForm)">
            <el-option v-for="item in nodeTypeOptions" :key="item.value" :label="item.label"
              :value="item.value"></el-option>
          </el-select>
        ...
        <el-form-item v-if="curBjData.task_type === 'detect'" label="使用节点数量" prop="nodeCount">
          <el-input v-model="curBjData.nodeCount" placeholder="请输入使用节点数量" style="width: 300px"
            @keyup.enter="handleSubmit2(bjForm)" />
        </el-form-item>
        <el-form-item v-if="curBjData.task_type === 'detect'" label="探测目标"
            prop="ip_targets">
            <el-input v-model="curBjData.ip_targets" placeholder="多个ip，以英文逗号分割" style="width: 340px"  :rows="4" type="textarea"
              @keyup.enter="handleSubmit2(bjForm)" />
          </el-form-item>
      </el-form>
      <template #footer>
...
import { ElMessage, type FormInstance, ElMessageBox, type UploadUserFile } from 'element-plus'
    ...
    if (valid) {
      ...
      if (curXjData.task_type == 'search') {
        ...
        let formData2 = {
              ...
              labelIds: toArray(curXjData.labelIds),
              taskContent: {
                ipSourceType: 1,
                // ip_targets: curXjData.ip_targets.split(','),
                port_targets: toArray(curXjData.port_targets),
                targetGroups: [],
                pocType: 1,
                poc_name: curXjData.poc_name,
                pocs: [curXjData.pocs],
                pocGroups: [],
                blacklist: toArray(curXjData.blacklist),
    ...
    if (valid) {
      ...
      if (curBjData.task_type == 'search') {
        ...
        formData = {
              ...
              // labelIds: curBjData.labelIds ? curBjData.labelIds.split(',') : [],
              // labelIds: curBjData.labelIds,
              labelIds: toArray(curBjData.labelIds),
              taskContent: {
                ipSourceType: 1,
                ip_targets: toArray(curBjData.ip_targets),
                port_targets: toArray(curBjData.port_targets),
                // ip_targets: curBjData.ip_targets,
                // port_targets: curBjData.port_targets,
                // ip_targets: curBjData.ip_targets.split(','),
                // port_targets: curBjData.port_targets.split(','),
                targetGroups: [],
                pocType: 1,
                poc_name: curBjData.poc_name,
                pocs: [curBjData.pocs],
                pocGroups: [],
                blacklist: toArray(curBjData.blacklist)
